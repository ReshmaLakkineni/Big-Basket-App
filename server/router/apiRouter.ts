...
apiRouter.post('/', async(request:express.Request , response:express.Response) => {
    try{
        ...
        //create if the same product exists
        let product = await ProductTable.findOne({name : name});
        if(product){
            return response.status(401).json({msg : 'Product is already exists!'});
        }
        //create a Product
...
apiRouter.put('/:productId', async(request:express.Request, response:express.Response) => {
    ...
    try{
        let {name, image, qty, price, info} = request.body;
...
apiRouter.get('/:productId', async(request:express.Request, response:express.Response) => {
    ...
    try{
        let product = await ProductTable.findById(productId);
        if(!product){
            return response.status(401).json({msg : 'Product is not found!'}); //send error
        }
        response.status(200).json({product : product}); //if not
    }
    catch(error){
        if(error.kind === 'ObjectId'){
            return response.status(500).json({msg : 'Product is not found!'}); //if id is improper we get proper msg
        }
        return response.status(500).json({msg : 'Server Error!'})
    }
});
