...
export const createProduct = (product:Product , history:any):any => {
    return async (dispatch:any) => {
        try{
            ...
            let response = await axios.post(dataURL, product); //type of request -> post req, for post along with URL, we need to provide form data
            dispatch({type : CREATE_PRODUCT_SUCCESS , payload : response.data});//successfully created, call the action
